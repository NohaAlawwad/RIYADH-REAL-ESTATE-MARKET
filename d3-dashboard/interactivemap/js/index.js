...
d3.queue(3)
      .defer(d3.csv, 'OffersPerdistrict_Villas.csv')
      .defer(d3.csv, 'avgperunitarea__Villa.csv')
...
function mapchart(yearval, mapdata){
    ...
    var legendval = [2000, 4000, 6000, 8000, 10000];
...
function linechart(nestedData){
	...
	for(var i = 0; i < nestedData.length; i++){
		...
		var orderval = nestedData[i].orderval;
    ...
    var pcolordata = [];
	for(var i = 0; i<nestedData.length; i++){
    	let pvals = nestedData[i].values[1][2014];
    	var oval = nestedData[i].orderval;
		pcolordata[i] = {
			priceval : pvals,
			orderval : oval
		}
    ...
    .enter().append("rect")
      ...
      .on("mouseover", function(d){
		...
		d3.select("#map").selectAll(".tooltip").style("visibility", "visible")
    		...
    		.style("top", (getcx - 220) + "px")
    		//.style("left", (getcy + 120) + "px")
